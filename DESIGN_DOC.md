...
- It creates a temporary folder on a **shared filesystem** for the job, writes a slurm job config for an array job to it, and a small harness script that runs the computation per task. The harness loads the cloudpickled payload (function + bound args + source specs), reopens the sources, gets the blocks for this task, runs them in a loop, and writes the per-task results and per-task success once per task (not once per block, to avoid swamping the filesystem with tiny files). It also writes a job **manifest** (submitted array/job ids, the block→task assignment, the temp folder) so a run can be *reattached* — if the orchestrating (login-node) process dies, a later call can pick the job back up from the manifest instead of resubmitting from scratch.
- After submitting the array, the runner watches the respective slurm jobs. It polls every 10 seconds
  ...
  (also reports how many jobs are pending, running, have succeeded, have failed). The **ground truth for success is the per-task result/sentinel files**, not the scheduler state: a task counts as done iff its result file exists. The scheduler is queried only to detect *dead* tasks — using `sacct` rather than `squeue` (squeue drops finished jobs, while sacct reports terminal states reliably: COMPLETED, FAILED, TIMEOUT, OUT_OF_MEMORY, NODE_FAIL, PREEMPTED, CANCELLED). A task that has left the scheduler in a terminal state but has no result file is treated as failed.
...
The protocol is shared; slurm is a thin layer. `SlurmRunner` subclasses `_DistributedRunner` and implements only `_launch_and_wait(tmp, n_tasks, num_workers, name)` (plus `reattach`) — everything else (payload, harness, result/sentinel collection, `_finalize` failure handling, `block_ids` re-run, `_require_reopenable`) is inherited and exercised by the `subprocess` backend. How it works and the gotchas it handles:

- **One array job.** It renders an sbatch script from `SlurmConfig` (partition, time, mem, cpus_per_task, gpus, account, qos, constraint, shebang) with `--array=0-(n_tasks-1)%<throttle>`. Array index = task id; the per-task command is exactly the harness invocation the subprocess backend uses, run with the absolute `python_executable` (default `sys.executable`). `num_workers` is the array throttle, decoupled from task count — the base partitions into `n_tasks` independently. `n_tasks` is guarded against the cluster's `MaxArraySize` (queried via `scontrol`, overridable with `SlurmConfig.max_array_size`) so an oversized run fails up front rather than at submit.
...
- **Re-run on failure** works via `block_ids` with no slurm-specific code: the raised `RunnerError` carries `failed_block_ids` and the preserved `tmp_folder`; re-invoking the op (or `runner.run`) with those `block_ids` re-partitions and resubmits only the failed blocks (a fresh temp folder per call).
- **Testing.** `subprocess` is the CI proxy for the protocol; `tests/test_slurm_runner.py` adds slurm-only tests (gated on `sbatch` + `BIOIMAGE_PY_SHARED_TMP`) asserting parity against `local`/`subprocess` plus failure+re-run and reattach end to end. Because pytest's `tmp_path` is node-local, both the test arrays and `tmp_root` are placed on the shared filesystem via the `shared_tmp_path` / `shared_zarr_factory` fixtures.
